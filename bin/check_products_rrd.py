...
def check_rrds(rrddir, max_age_minutes, max_age_intervals=2.0,
    ...
    for rrdfile in files:
        ...
        if test_ref_datetime is None:
            # should be default
            reftime_dt = datetime.utcnow() - timedelta(seconds=current_max_age_minutes*60)
        else:
            # only for debugging purposes
            reftime_dt = test_ref_datetime - timedelta(seconds=current_max_age_minutes*60)
            
        reftime = to_unix_seconds(reftime_dt)
        # reftime = to_unix_seconds(datetime.utcnow()) - (current_max_age_minutes*60)
        name = extract_prod_name(rrdfile)
        last = rrd.last(rrdfile)
        # print "name: " + name + " reftime: " + str(reftime) + " last: " + str(last) + " max_age_minutes: " + str(current_max_age_minutes) + " steps: " + str(step) + " max_age_intervals: " + str(max_age_intervals)
        if last < reftime:
            if sat_avail_service:
                res = sat_avail_service.get_data_availability_error(reftime_dt, name)
                if res:
...
def write_result_and_exit(code, message, cache_file):
    ...
    if cache_file is not None:
        ...
        try:
            ...
            os.chmod(cache_file, os.stat(cache_file).st_mode | stat.S_IRGRP | stat.S_IROTH)
